...
bool receiveDataContinuous(lora_packet_t *pkt)
{
    if (pkt == NULL)
    {
        return false;
    ...
    {
        pkt->len = sizeof(pkt->payload) - 1;
    }

    llcc68_read_char(pkt->payload, pkt->len);
    pkt->payload[pkt->len] = '\0';

    pkt->rssi = llcc68_packetRssi();
    pkt->snr = (int8_t)llcc68_snr();

    LOG_INF("Message: %s", pkt->payload);
    LOG_INF("RSSI=%d SNR=%d", pkt->rssi, pkt->snr);
